...
def get_student_list(major: str, groups: int, students : int) -> list:
    ...
    marks = list(map(lambda x : (TestLevel.vals()[x.level], x.level, x.test_mark == 0 and x.speaking_mark == 'A1m', x.first_name, x.second_name), marks))
    ...
    return list(map(lambda x : list(map(lambda y : (y[4], y[3], y[1]), x)), g))
