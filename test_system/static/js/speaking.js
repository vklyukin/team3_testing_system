...
function TableFill (q_table_body, json){
...
function changeEventHandlerPOST(event) {
...
function changeEventHandlerPUT(event) {
  let selected = select.options[select.selectedIndex].value;
...
const SendPut = (id) => { //function that checks for the changes in the questions and send them to the server side
  ...
  fetch(BASE_PATH + 'api/mark/' + id + '/', { //sending fetch put request to add changed question to the Data Base
      ...
      body:JSON.stringify({pk: id, major: major, user: user, test_mark: test_mark, test_level: test_level_selected, removed: removed, speaking: speaking, speaking_mark: speaking_mark_selected, level: level_selected, first_name: first_name, second_name: second_name, room: room, position: -1, confident: confident,})
    }).then(function (response) {
      if(response.status === 200){
        location.reload();
      }
